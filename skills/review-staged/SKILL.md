...
1. Load (in order, skip missing): `CLAUDE.md` at repo root + any nested `CLAUDE.md` it references; `AGENTS.md` + nested; all `~/.claude/rules/*.md`; all `docs/rules/*.md`. **Get the file list (step 3) first, then drop any rule file whose YAML frontmatter `paths` globs match none of the reviewable files** — a missing `paths` key means the file always applies. Otherwise a PHP rulebook rides along into a TypeScript review and reviewers cite rules that don't govern the diff.
2. Build a **manifest** of which sources loaded, which were missing, and which were skipped as not applicable — all three go in the report header. If zero rule files load, fall back to universal mode (correctness / security / performance only) and tag every `rule_source` as `"universal"`.
