...
| Скилл | Что делает |
|---|---|
| [`mr-ready`](skills/mr-ready) | Автономный цикл подготовки MR к ревью человеком: `review-staged` → находки тредами в MR (`🤖 self-review`) → `audit-reply --auto --commit` → push, ответы в треды → повтор до чистого ревью (кап 10 раундов). Спрашивает только продуктовые развилки. Единственный скилл, который коммитит, пушит и постит. |
...
```
/goal MR из отчёта mr-ready готов: последний раунд review-staged без открытых находок, проверки зелёные, ветка запушена, ответы в тредах — либо mr-ready сам сказал «стоп»
```
...
URL необязателен — найдётся по ветке; `--no-spec`, если чек-листа нет. Один пакет вопросов в
начале, потом тишина до продуктовой развилки или отчёта. `/goal clear` — остановить. Без `/goal`
скилл тоже работает.
